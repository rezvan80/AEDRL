...
from rl4co.utils.load_excel import load_data
...
class ConstructivePolicy(nn.Module):
    ...
    def forward(
        ...
        **decoding_kwargs,
        ...
        step = 0
        if (self.__module__ == "rl4co.models.zoo.clh_pomo.policy") |(self.__module__ == "rl4co.models.zoo.clh.policy"):
        ...
        if phase == "test":
            ...
            action_temp=actions[td["reward"].max(-1).indices]
            file=env.test_file.rsplit("/",maxsplit=1)[1].rsplit(".",maxsplit=1)[0]
            reward_out=td["reward"].max()
            charge=td["charge"][td["reward"].max(-1).indices]
            algorithm=self.__module__.rsplit(".",maxsplit=2)[1]
            load_data(action_temp,reward_out,charge,file,algorithm) 
        if return_actions:
...
def get_solution(td, k):
    ...
    max_length = 3.0
    batch_index = torch.arange(batch_size, device=data.device, dtype=torch.int32)[
        :, None
    ]
    demand = torch.nn.functional.pad(td["demand"][:batch_size], (1, 0), mode="constant", value=0)
    ...
    while (visited.all(-1) != 1).any() | (selected_node != 0).any():
        ...
        exceed_length_loc = (
            (
                (
                    data[batch_index, selected_node]
                    - data[:, 1 + station_size :]
                )
                .norm(p=2, dim=-1)
                .unsqueeze(-1)
                + (
                    data[:, 1 + station_size :, None]
                    - data[:, None, : 1 + station_size]
                ).norm(p=2, dim=-1)
                + current_length[:, :, None]
            )
            + 1e-4
        ...
        current_length += (
            data[batch_index, selected_node]
            - data[batch_index, next_selected_node]
        ).norm(p=2, dim=-1)

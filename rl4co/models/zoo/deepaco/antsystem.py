...
class AntSystem:
    ...
    def heuristic_dist(self) -> torch.Tensor:
        ...
        heuristic_dist[
            :,
            torch.arange(heuristic_dist.shape[1]),
            torch.arange(heuristic_dist.shape[2]),
        ] = 0
    ...
    def _sampling(
        ...
        # p = phe**alpha * heu**beta <==> log(p) = alpha*log(phe) + beta*log(heu) self.log_heuristic torch.log(self.log_heuristic)
        ...
        decode_strategy = Sampling(
            ...
            select_start_nodes_fn=partial(
                self.select_start_node_fn, start_node=self.start_node
            ),
        )
    ...
    def local_search(
        ...
        best_actions = env.local_search(
            td=td_cpu, actions=actions, **self.local_search_params
        )
        best_rewards = env.get_reward(td_cpu, best_actions)

        if self.use_nls:
            td_cpu_perturb = td_cpu.clone()
            td_cpu_perturb["distances"] = torch.tile(
                self.heuristic_dist, (self.n_ants, 1, 1)
            )
            ...
            for _ in range(self.n_perturbations):
                ...
                new_actions = env.local_search(
                    td=td_cpu, actions=perturbed_actions, **self.local_search_params
                )
    ...
    def _update_results(self, actions, reward):
        ...
        best_log=self.get_logp()[0][self._batchindex, best_index]
        if self.final_actions is None or self.final_reward is None:
            ...
            self.logp=list(iter(best_log.clone()))
        else:
            ...
            for index in require_update:
                ...
                self.logp[index]=best_log[index]

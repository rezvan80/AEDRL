...
class DeepACOPolicy(NonAutoregressivePolicy):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # heatmap_logits=self.calculate_distance(td_initial)
    ...
    def calculate_distance(self, td_initial):
        ...
        # diagonal_mask = torch.eye(distance_matrix.size(1)).to(distance_matrix.device)
        # distance_matrix = distance_matrix + diagonal_mask * 1e-6
        # reciprocal_distance_matrix = torch.reciprocal(distance_matrix)
        # result = reciprocal_distance_matrix - diagonal_mask * reciprocal_distance_matrix
        diagonal_mask = (
            torch.eye(distance_matrix.size(1)).to(distance_matrix.device) * 1e-9
        )
        distance_matrix = distance_matrix + diagonal_mask
        reciprocal_distance_matrix = torch.reciprocal(distance_matrix)
        result = reciprocal_distance_matrix - diagonal_mask  # 对角线非0

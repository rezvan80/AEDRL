...
class AttentionModelDecoder(AutoregressiveDecoder):
    ...
    def forward(
        ...
        mask = td["action_mask"]
        # if (~mask).all(-1).any():
        #     index=(~mask).all(-1).unsqueeze(-1).expand_as(mask).clone()
        #     index[...,-td["demand"].size(-1):]=False
        #     index[...,0]=False
        #     batch_index=torch.arange(mask.size(0),device=mask.device,dtype=torch.int64)[:,None]
        #     index[batch_index,td["current_node"]]=False
        #     mask[index]=True

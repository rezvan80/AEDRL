...
def env_init_embedding(env_name: str, config: dict) -> nn.Module:
    ...
    embedding_registry = {
        ...
        "evrp": EVRPInitEmbedding,
        "evrptw": EVRPTWInitEmbedding,
...
class EVRPInitEmbedding(nn.Module):
    def __init__(self, embed_dim, linear_bias=True, node_dim: int = 3):
        super(EVRPInitEmbedding, self).__init__()
        node_dim = node_dim  # 3: x, y, demand
        self.init_embed = nn.Linear(node_dim, embed_dim, linear_bias)
        self.init_embed_depot = nn.Linear(2, embed_dim, linear_bias)  # depot embedding
        self.init_embed_station = nn.Linear(2, embed_dim, linear_bias)

    def forward(self, td):
        # [batch, 1, 2]-> [batch, 1, embed_dim]
        station_size = td["locs"].size(-2) - 1 - td["demand"].size(-1)
        depot, stations, cities = (
            td["locs"][:, :1, :],
            td["locs"][:, 1 : 1 + station_size, :],
            td["locs"][:, 1 + station_size :, :],
        )
        depot_embedding = self.init_embed_depot(depot)
        # [batch, n_city, 2, batch, n_city, 1]  -> [batch, n_city, embed_dim]
        node_embeddings = self.init_embed(
        ...
        station_embeddings = self.init_embed_station(stations)
        # [batch, n_city+1, embed_dim]
        out = torch.cat((depot_embedding, station_embeddings, node_embeddings), -2)
        return out


class EVRPTWInitEmbedding(nn.Module):
    def __init__(self, embed_dim, linear_bias=True, node_dim: int = 6):
        super(EVRPTWInitEmbedding, self).__init__()
        node_dim = node_dim  # 3: x, y, demand
        self.init_embed = nn.Linear(node_dim, embed_dim, linear_bias)
        self.init_embed_depot = nn.Linear(2, embed_dim, linear_bias)  # depot embedding
        self.init_embed_station = nn.Linear(4, embed_dim, linear_bias)
    ...
    def forward(self, td):
        ...
        node_embeddings = self.init_embed(
            torch.cat(
                (
                    cities,
                    td["durations"][:, 1 + station_size :, None],
                    td["time_windows"][:, 1 + station_size :],
                    td["demand"][..., None],
                ),
                -1,
            )
        )
        station_embeddings = self.init_embed_station(
            torch.cat(
                (
                    stations,
                    td["durations"][:, 1 : 1 + station_size, None],
                    td["time_windows"][:, 1 : 1 + station_size, 1:],
                ),
                dim=-1,
            )
        )

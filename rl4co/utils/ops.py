...
def get_num_starts(td, env_name=None):
    ...
    elif env_name in ["evrp", "evrptw"]:
        num_starts = td["demand"].size(-1)
...
def select_start_nodes(td, env, num_starts):
    ...
    """
    num_loc = td["demand"].size(-1) if hasattr(env.generator, "num_loc") else 0xFFFFFFFF
    ...
    elif env.name in ["evrp", "evrptw"]:
        selected = torch.arange(
            start=num_station + 1, end=num_loc + num_station + 1, device=td.device
        ).repeat_interleave(td.batch_size[0])
    else:

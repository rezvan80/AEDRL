...
class EVRPEnv(RL4COEnvBase):
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        visited.scatter_(-1, torch.clamp(current_node - num_station, 0, n_loc), 1)
        visit = visited[:, 1:].clone()
        charge = td["charge"]
        charge[(current_node == 0) & (used_length > 0)] += (
            3 - used_length[(current_node == 0) & (used_length > 0)]
        )

        used_length[(current_node <= num_station) & (current_node > 0)] = torch.where(
            used_length[(current_node <= num_station) & (current_node > 0)] >= 0.5,
            used_length[(current_node <= num_station) & (current_node > 0)] - 0.5,
            torch.tensor(0),
        )
        used_length[current_node == 0] = 0
    ...
    def get_action_mask(td: TensorDict) -> torch.Tensor:
        ...
        mask_temp = torch.nn.functional.pad(
            mask_station, (1, 0), mode="constant", value=0
        )
        current_node = torch.where(
            (td["current_node"] > 0)
            & (td["current_node"] <= num_station)
            & (td["used_length"] > 1e-4),
            td["current_node"],
            torch.tensor(0),
        )
        mask_temp[batch_index, current_node] = False
        mask_station = mask_temp[:, 1:].clone()
